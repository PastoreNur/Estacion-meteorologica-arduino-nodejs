...
function buscarfecha(params) {
    ...
    entradas.forEach(element => {
        if (element.time.Dia == params.Dia && element.time.Mes == params.Mes && element.time.anio == params.anio) {
...
function buscarhora(params) {
    const porhora= [];
    entradas.forEach(element => {
        if (element.time.Dia == params.Dia && element.time.Mes == params.Mes && element.time.anio == params.anio && element.time.hora == params.hora) {
...
function buscarrango(inicial,final) {
    const porrango= [];
    entradas.forEach(element => {
        if ((element.time.Dia >= inicial.Dia && element.time.Dia <= final.Dia) && (element.time.Mes >= inicial.Mes && element.time.Mes <= final.mes)&& (element.time.anio >= inicial.anio && element.time.anio <= final.anio )) {
...
module.exports= {
    escribir,buscarfecha,promediofecha,buscarhora,promediohora,buscarrango,promediorango
}